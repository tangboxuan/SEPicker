...
def index():
    ...
    if request.method == "GET":
        ...
        for country in country_first_dict:
            bool = False
            for uni in country_first_dict[country]:
                if country_first_dict[country][uni]["n_mods"] != 0:
                    bool = True
                    break
            if bool:
                filtered[country] = country_first_dict[country]


        school_mappings = {};
        for country in country_first_dict:
            for school in country_first_dict[country]:
                school_mappings[school] = "".join(u for u in school if u not in ("?", ".", ";", ":", "!","'", ","));


        return render_template("picker.html", output_dict = filtered, str_of_nusmods=str_nusmods,
